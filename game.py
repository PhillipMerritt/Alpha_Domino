...
class Game:
    ...
    def _generate_board(self, turn):
        ...
        board[3][24] = 0    # everything from here to the return statement is generating a specific board state for debugging
        ...
        if not dub_found:                       # if not choose a random domino from their hand and play that
            choices = np.nonzero(board[1])[0]   # this is a pretty bad solution for this that should be updated at some point
            choice = np.random.choice(choices)
            board[1][choice] = 0

            dom = self.gameState.to_domino(choice)  # converts the index of the randomly chosen domino to a tuple of its pip values

            choice = np.random.choice(dom)          # choose 1 of the two pip values randomly

            head_index = self.gameState.head_index(choice)  # convert the chosen pip value into the index of the double domino of that value
    ...
    def step(self, action):`    # game state makes the chosen action and returns the next state, the value of the next state for the active player in that state, and whether or not the game is over
        next_state, value, done = self.gameState.takeAction(action)     # value is always 0 unless the game is over. Otherwise it is -1 since the last player made a winning move
...
class GameState():
    def __init__(self, board, playerTurn):
        ...
        self.all_domino = [(0,0),(0,1),(1,1),(0,2),(1,2),(2,2),(0,3),(1,3),(2,3),(3,3),(0,4),(1,4),(2,4),(3,4),(4,4),(0,5),(1,5),(2,5),(3,5),(4,5),(5,5),(0,6),(1,6),(2,6),(3,6),(4,6),(5,6),(6,6)]
        self.head_values = {0: 0, 2: 1, 5: 2, 9: 3, 14: 4, 20: 5, 27: 6}    # head_values is a dict with the head_indices (doubles) as keys and the corresponding head values as values
    ...
    def _allowedActions(self):
        ...
        while 1:    # checks for actions. If none found draw a domino and try again
            ...
            for index in hand:
                domino = self._to_domino(np.int(index)) # convert each index in the hand to a domino tuple
    ...
    def _convertStateToId(self): # appends a list of the nonzero indices from each of the board lists one after the other
        position = np.append(np.nonzero(self.board[0])[0],np.nonzero(self.board[1])[0])
        position = np.append(position,np.nonzero(self.board[2][0]))
        position = np.append(position,np.nonzero(self.board[3])[0])
    ...
    def _getScore(self):    # I was tinkering with this for a bit and this version is pretty arbitrary. Basically the fewer dominos in your hand the higher your score
    ...
    def _convert_head(self, action, newBoard):
        played_domino = self._to_domino(action) # get pip tuple of the chosen action
